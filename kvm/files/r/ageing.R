...
aged_samples_lr <- function( attributes , season , orig_data , correction , mean_coef , plot ){
	...
	for ( attr in intersect( attributes , AGEING_AVAILABLE_ATTRS ) ){
		...
		aged_samples <- read.csv( blank.lines.skip=FALSE, file = aged_samples_file_name , header = FALSE , sep = "" , col.names = c( "time" , "value" )  )
...
age_instance <- function( instance , time , age_lr ){
	...
	ageing_related_attrs <- intersect( intersect( AVAILABLE_ATTRS , AGEING_AVAILABLE_ATTRS ) , colnames( instance ) )
	
	# applying log10 to the ageing related attributes since ageing information (age_lr) is in logarithmic scale
	instance[ , ageing_related_attrs ] = log10( instance[ , ageing_related_attrs ] + EPSILON )
	
	# for each one of the available attributes from which ageing data is available
	for ( attr in ageing_related_attrs){
	...
	instance[ , ageing_related_attrs ] = 10 ^ instance[ , ageing_related_attrs ]
	instance
}
###############################################################################################################################################################
