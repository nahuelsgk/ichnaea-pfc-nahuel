...
\begin{itemize}
\item Nom de la matriu com a enlla� per veure la matriu
...
\item Editar la variable a la que es vol afegir la variable
\item Al apartat inferior ''List of season set'', clickar a la icona de suma
\end{itemize}
Es poden donar un nom i afegir tants fitxers com es vulguin. S'ha de tenir cura perque exiteix la possibilitat de afegir varis fitxers a una mateixa estaci\'{o}. Actualment Ichnaea solament processa estiu e hivern. Per m\'{e}s descripci\'{o} d' aquesta interf\'{i}cie anar al punt \ref{season_set:edit}
...
\item Llistar les variables per seleccionar una variable('Ichnaea Data - View variables')
\item Editar la variable a la que es vol afegir la variable
\item Editar, amb la icona d'edici\'{o}, la ''season set'' del llistat ''List of season set''.
...
On:
\begin{itemize}
\item Alias de la columna: \'{e}s un nom qualsevol per identificar la columna. Si el sistema cont\'{e} una variable amb el mateix nom, autom\`{a}ticament li assignare aquesta variable amb una ''season set'' per defecte.
...
A la secci\'{o} de les cap�aleres, a la icona del llapis, es pot especificar un alias a la columna. Si es prem "Enter" o es canvia el focus, s'activa el bot\'{o} de salvaguardat.
...
\section{Llistar trainings d'una matriu}
Pendent de implementaci\'{o}
