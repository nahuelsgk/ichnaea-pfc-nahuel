...
On:
\begin{itemize}
\item Alias de la columna: \'{e}s un nom qualsevol per identificar la columna. Si el sistema cont\'{e} una variable amb el mateix nom, autom\`{a}ticament li assignare aquesta variable amb una ''season set'' per defecte. Si l'alias de variable definit al full de c\`{a}lcul \'{e}s el mateix que el d'una variable del sistema, s'assigna a la variable i li assignar un conjunt de fitxers per defecte.
...
\begin{figure}[h!]
  ...
  \label{fig:placement}
\end{figure}
Acceptant, es clona la matriu i anem a la interficie de configuraci\Â´{o}. Mirar \ref{sec:configure_matrix}.
...
Si la creaci\Â´{o} \Â´e{s} correcte, les dades s'enviaran a la cua de procesos i la aplicaci\Â´{o} es redirigir\Â´{a} la pantalla de visualitzaci\Â´{o} de trainings.
...
Desde la casa de l'usuari(mirar \ref{sec:home}), es pot veure els teus trainings i en quin estadi es troben. Amb la icona "ull", pots accedir a visualitzar la informaci\Â´{o} del training.
